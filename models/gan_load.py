...
class StyleGAN2Wrapper(nn.Module):
    ...
    def forward(self, input, cond = torch.empty(0, 3), input_is_latent=False):
        return self.style_gan2(input, cond, input_is_latent=input_is_latent)[0]

    def gen_shifted(self, z, shift):
        cond = torch.empty(0, 3)
        w = self.style_gan2.mapping(torch.unsqueeze(z, dim=0), cond)
        print('z:', z.shape)
        print('w:', w.shape)
        ...
        if self.shift_in_w:
            ...
            return self.forward(torch.unsqueeze(z + shift, dim=0), cond, input_is_latent=False)
...
def make_style_gan2(size, weights, shift_in_w=True):
    G = StyleGAN2Generator(z_dim = 512, c_dim = 0, w_dim = 512, img_resolution = 256, img_channels = 3, mapping_kwargs = {}, channel_base = 16384, num_layers=14, num_critical=2, margin_size=10, num_fp16_res=4)
